...
zoom_plot_title_fontsize = config.getint('FONTS', 'zoom_plot_title')
zoom_plot_label_fontsize = config.getint('FONTS', 'zoom_plot_labels')
zoom_plot_axis_fontsize = config.getint('FONTS', 'zoom_plot_ticks')
zoom_plot_time_tick_interval = config.getint('AXIS_TICKS', 'zoom_time_tick_interval')
zoom_plot_x_limits = [int(x.strip()) for x in config.get('SETTINGS', 'zoom_plot_x_limits').split(',')]
...
deck_zoom_plot_title_fontsize = config.getint('FONTS', 'deck_zoom_plot_title')
deck_zoom_plot_label_fontsize = config.getint('FONTS', 'deck_zoom_plot_labels')
deck_zoom_plot_axis_fontsize = config.getint('FONTS', 'deck_zoom_plot_ticks')
deck_zoom_plot_time_tick_interval = config.getint('AXIS_TICKS', 'zoom_time_tick_interval')
deck_zoom_plot_legend_fontsize = config.getint('FONTS', 'deck_zoom_plot_legend')
...
default_ylim = [int(y.strip()) for y in config.get('AXIS_TICKS', 'default_y_range').split(',')]
default_yticks_interval = config.getint('AXIS_TICKS', 'default_y_tick_interval')
manual_ylimits = {} # This would need more complex parsing if used in config.ini
...
if generate_individual_plots:
    ...
    for component_name in all_component_names:
        ...
        ax_full.set_title(display_name.replace('_', ' '), fontsize=full_plot_title_fontsize, **font_settings)
        ...
        ax_full.tick_params(axis='y', labelsize=full_plot_axis_fontsize)
        ...
        if component_name in manual_ylimits:
            ...
            ax_full.set_yticks(np.arange(manual_ylimits[component_name][0], manual_ylimits[component_name][1] + 1, default_yticks_interval).astype(int))
        elif sim_min >= default_ylim[0] and sim_max <= default_ylim[1]:
            ax_full.set_ylim(default_ylim)
            ax_full.set_yticks(np.arange(default_ylim[0], default_ylim[1] + 1, default_yticks_interval).astype(int))
        else:
            print(f"  WARNING: Data for '{component_name}' exceeds default Y-limits. Auto-adjusting.")
            ylim_lower, ylim_upper = np.floor(sim_min), np.ceil(sim_max)
            ax_full.set_ylim(ylim_lower, ylim_upper)
            ax_full.set_yticks(np.unique(np.round(np.linspace(ylim_lower, ylim_upper, 7))).astype(int))
        for spine in ax_full.spines.values():
            spine.set_edgecolor('black')
            spine.set_linewidth(1.5)
        plt.tight_layout()
        fig_full.savefig(os.path.join(output_folder_full, f'Plot_Full_{clean_name}.png'), dpi=300)
        plt.close(fig_full)

        # Zoomed Profile Plot
        zoom_mask = (time_fine >= zoom_plot_x_limits[0]) & (time_fine <= zoom_plot_x_limits[1])
        if not np.any(zoom_mask): continue
        time_zoomed_fine = time_fine[zoom_mask]
        data_zoomed_interp = component_data_interp[zoom_mask]
        data_zoomed_raw = component_data_raw[(local_time_minutes >= zoom_plot_x_limits[0]) & (local_time_minutes <= zoom_plot_x_limits[1])]
        ...
        ax_zoomed.set_title(f"{display_name.replace('_', ' ')}", fontsize=zoom_plot_title_fontsize, **font_settings)
        ...
        start_tick = np.floor(zoom_plot_x_limits[0] / zoom_plot_time_tick_interval) * zoom_plot_time_tick_interval
        xticks_zoomed = np.arange(start_tick, zoom_plot_x_limits[1], zoom_plot_time_tick_interval)
        xticks_zoomed = np.append(xticks_zoomed, zoom_plot_x_limits[1])
        ...
        ax_zoomed.tick_params(axis='y', labelsize=zoom_plot_axis_fontsize)
        plt.setp(ax_zoomed.get_xticklabels(), fontweight="bold")
        plt.setp(ax_zoomed.get_yticklabels(), fontweight="bold")
        ax_zoomed.set_xlim(zoom_plot_x_limits)
        ylim_lower, ylim_upper = np.floor(data_zoomed_interp.min()), np.ceil(data_zoomed_interp.max())
        ax_zoomed.set_ylim(ylim_lower, ylim_upper)
        ax_zoomed.set_yticks(np.unique(np.round(np.linspace(ylim_lower, ylim_upper, 7))).astype(int))
        ...
        for spine in ax_zoomed.spines.values():
            ...
            spine.set_linewidth(1.5)
        plt.tight_layout()
        fig_zoomed.savefig(os.path.join(output_folder_zoomed, f'Plot_Zoomed_{clean_name}.png'), dpi=300)
...
with open(report_filepath, 'w') as report_file:
    ...
    for deck_name, component_list in deck_groups.items():
        ...
        ax_deck.tick_params(axis='y', labelsize=deck_plot_axis_fontsize)
        ...
        if deck_name in manual_ylimits:
             ...
             ax_deck.set_yticks(np.arange(manual_ylimits[deck_name][0], manual_ylimits[deck_name][1] + 1, default_yticks_interval).astype(int))
        elif deck_min >= default_ylim[0] and deck_max <= default_ylim[1]:
            ax_deck.set_ylim(default_ylim)
            ax_deck.set_yticks(np.arange(default_ylim[0], default_ylim[1] + 1, default_yticks_interval).astype(int))
        else:
            print(f"  WARNING: Data for deck '{deck_name}' exceeds default Y-limits. Auto-adjusting.")
            ylim_lower, ylim_upper = np.floor(deck_min), np.ceil(deck_max)
            ax_deck.set_ylim(ylim_lower, ylim_upper)
            ax_deck.set_yticks(np.unique(np.round(np.linspace(ylim_lower, ylim_upper, 7))).astype(int))
        ...
        fig_deck.savefig(os.path.join(output_folder_consolidated, f'Plot_Deck_{clean_deck_name}.png'), dpi=300)
        plt.close(fig_deck)

        # --- Format and Save Zoomed Consolidated Plot ---
        ...
        start_tick = np.floor(zoom_plot_x_limits[0] / deck_zoom_plot_time_tick_interval) * deck_zoom_plot_time_tick_interval
        xticks_zoomed = np.arange(start_tick, zoom_plot_x_limits[1], deck_zoom_plot_time_tick_interval)
        xticks_zoomed = np.append(xticks_zoomed, zoom_plot_x_limits[1])
        ...
        ax_deck_zoomed.tick_params(axis='y', labelsize=deck_zoom_plot_axis_fontsize)
        plt.setp(ax_deck_zoomed.get_xticklabels(), fontweight="bold")
        plt.setp(ax_deck_zoomed.get_yticklabels(), fontweight="bold")
        ax_deck_zoomed.set_xlim(zoom_plot_x_limits)
        zoom_start_index_fine = np.where(time_fine >= zoom_plot_x_limits[0])[0][0]
        ...
        ylim_lower, ylim_upper = np.floor(deck_zoomed_min), np.ceil(deck_zoomed_max)
        ax_deck_zoomed.set_ylim(ylim_lower, ylim_upper)
        ax_deck_zoomed.set_yticks(np.unique(np.round(np.linspace(ylim_lower, ylim_upper, 7))).astype(int))
        ...
        fig_deck_zoomed.savefig(os.path.join(output_folder_consolidated_zoomed, f'Plot_Deck_Zoomed_{clean_deck_name}.png'), dpi=300)
